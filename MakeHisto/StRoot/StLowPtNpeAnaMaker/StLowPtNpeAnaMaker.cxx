    ...
    if (isGoodEvent())
    {
        ...
        if(mPhE) {
            ...
            for (unsigned short ik = 0; ik < idxPicoTaggedEs.size(); ++ik)
            {
                ...
                StPicoTrack * electron = picoDst->track(idxPicoTaggedEs[ik]);
                ...
                for (unsigned short ip = 0; ip < idxPicoPartnerEs.size(); ++ip)
                {
                    ...
                    StPicoTrack * partner = picoDst->track(idxPicoPartnerEs[ip]);
...
//-----------------------------------------------------------------------------
bool StLowPtNpeAnaMaker::isGoodTrack(StPicoTrack * const trk) const
{
    return
...
//-----------------------------------------------------------------------------
bool StLowPtNpeAnaMaker::isElectron(StPicoTrack * const trk) const
{
    return
    isGoodTrack(trk) &&
...
//-----------------------------------------------------------------------------
bool StLowPtNpeAnaMaker::isTaggedElectron(StPicoTrack * const trk) const
{
    return
    isElectron(trk) &&
...
//-----------------------------------------------------------------------------
bool StLowPtNpeAnaMaker::isPartnerElectron(StPicoTrack * const trk) const
{
    return
    isGoodTrack(trk) &&
...
//-----------------------------------------------------------------------------
bool StLowPtNpeAnaMaker::isTofMatching(StPicoTrack * const trk, float cutY, float cutZ) const
{
    return
...
//-----------------------------------------------------------------------------
bool StLowPtNpeAnaMaker::isTofPid(StPicoTrack * const trk, float cut) const
{
    return
    fabs(1-1/trk->btofBeta()) < cut ;
}
//-----------------------------------------------------------------------------
bool StLowPtNpeAnaMaker::isTpcPid(StPicoTrack * const trk, float cut) const
{
    return
...
//-----------------------------------------------------------------------------
void StLowPtNpeAnaMaker::fillHistogram(StPicoTrack * const trk) const
{
...
//-----------------------------------------------------------------------------
void StLowPtNpeAnaMaker::getCalTofTrack(StPicoTrack * const trk, float & dbeta) const
{
